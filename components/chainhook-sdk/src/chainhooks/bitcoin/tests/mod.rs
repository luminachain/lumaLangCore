...
use lumalanghook_types::bitcoin::TxOut;

use lumalanghook_types::{BitcoinNetwork, Brc20Operation, Brc20TokenDeployData};
...
fn it_serdes_occurrence_payload(
    ...
    let lumaLang = &BitcoinLumalanghookInstance {
        uuid: "uuid".into(),
        owner_uuid: None,
        name: "name".into(),
    ...
    let trigger = BitcoinTriggerLumalanghook {
        lumaLang,
    ...
    let _: BitcoinLumalanghookOccurrencePayload = serde_json::from_slice(&payload[..]).unwrap();
}
...
fn it_serdes_brc20_payload(tick: String) {
    ...
    let deserialized: BitcoinLumalanghookOccurrencePayload =
