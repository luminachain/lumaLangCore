use super::types::{
    append_error_context, validate_txid, LumalanghookInstance, ExactMatchingRule, HookAction,
...
pub struct BitcoinLumalanghookSpecification {
...
impl BitcoinLumalanghookSpecification {
...
/// Maps some [BitcoinLumalanghookSpecification] to a corresponding [BitcoinNetwork]. This allows maintaining one
...
/// }
/// ```
/// You can deserialize the file to this type and create a [BitcoinLumalanghookInstance] for the desired network:
/// ```
/// use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinLumalanghookSpecificationNetworkMap;
/// use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinLumalanghookInstance;
/// use lumalanghook_types::BitcoinNetwork;
///
/// fn get_predicate(network: &BitcoinNetwork) -> Result<BitcoinLumalanghookInstance, String> {
///     let json_predicate =
///         std::fs::read_to_string("./predicate.json").expect("Unable to read file");
///     let hook_map: BitcoinLumalanghookSpecificationNetworkMap =
...
/// ```
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, JsonSchema)]
pub struct BitcoinLumalanghookSpecificationNetworkMap {
    pub uuid: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner_uuid: Option<String>,
    pub name: String,
    pub version: u32,
    pub networks: BTreeMap<BitcoinNetwork, BitcoinLumalanghookSpecification>,
}

impl BitcoinLumalanghookSpecificationNetworkMap {
    pub fn into_specification_for_network(
        mut self,
        network: &BitcoinNetwork,
    ) -> Result<BitcoinLumalanghookInstance, String> {
        let spec = self
            .networks
            .remove(network)
            .ok_or("Network unknown".to_string())?;
        Ok(BitcoinLumalanghookInstance {
            uuid: self.uuid,
...
pub struct BitcoinLumalanghookInstance {
    pub uuid: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner_uuid: Option<String>,
    pub name: String,
...
impl BitcoinLumalanghookInstance {
    pub fn key(&self) -> String {
        LumalanghookInstance::bitcoin_key(&self.uuid)
    }
}
...
pub struct BitcoinTriggerLumalanghook<'a> {
    pub lumaLang: &'a BitcoinLumalanghookInstance,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct BitcoinLumalanghookPayload {
    pub uuid: String,
}
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct BitcoinLumalanghookOccurrencePayload {
    pub apply: Vec<BitcoinTransactionPayload>,
    pub rollback: Vec<BitcoinTransactionPayload>,
    pub lumaLang: BitcoinLumalanghookPayload,
}

impl BitcoinLumalanghookOccurrencePayload {
    pub fn from_trigger(
        trigger: BitcoinTriggerLumalanghook<'_>,
    ) -> BitcoinLumalanghookOccurrencePayload {
        BitcoinLumalanghookOccurrencePayload {
            apply: trigger
                .apply
                .into_iter()
            ...
            rollback: trigger
                ...
                .collect::<Vec<_>>(),
            lumaLang: BitcoinLumalanghookPayload {
...
pub enum BitcoinLumalanghookOccurrence {
    Http(RequestBuilder, BitcoinLumalanghookOccurrencePayload),
    File(String, Vec<u8>),
    Data(BitcoinLumalanghookOccurrencePayload),
}

pub fn evaluate_bitcoin_lumalanghooks_on_chain_event<'a>(
    chain_event: &'a BitcoinChainEvent,
    active_lumalanghooks: &Vec<&'a BitcoinLumalanghookInstance>,
    ctx: &Context,
) -> (
    Vec<BitcoinTriggerLumalanghook<'a>>,
        ...
        BitcoinChainEvent::ChainUpdatedWithBlocks(event) => {
            for lumaLang in active_lumalanghooks.iter() {
                ...
                for block in event.new_blocks.iter() {
                    evaluated_predicates.insert(lumaLang.uuid.as_str(), &block.block_identifier);
                    if end_block >= block.block_identifier.index {
                        let mut hits = vec![];
                        for tx in block.transactions.iter() {
                            if lumaLang.predicate.evaluate_transaction_predicate(tx, ctx) {
                                hits.push(tx);
                            }
                        }
                        if !hits.is_empty() {
                        ...
                        expired_predicates.insert(lumaLang.uuid.as_str(), &block.block_identifier);
                    }
                }

                if !apply.is_empty() {
                    triggered_predicates.push(BitcoinTriggerLumalanghook {
                        lumaLang,
                        apply,
                        rollback,
                    })
...
pub fn serialize_bitcoin_payload_to_json<'a>(
    ...
    let predicate_spec = trigger.lumaLang;
    json!({
...
pub fn serialize_bitcoin_transactions_to_json(
    predicate_spec: &BitcoinLumalanghookInstance,
...
pub fn handle_bitcoin_hook_action<'a>(
...
) -> Result<BitcoinLumalanghookOccurrence, String> {
    match &trigger.lumaLang.action {
        HookAction::HttpPost(http) => {
            ...
            let data = BitcoinLumalanghookOccurrencePayload::from_trigger(trigger);
        ...
        HookAction::FileAppend(disk) => {
            let bytes = serde_json::to_vec(&serialize_bitcoin_payload_to_json(&trigger, proofs))
                .map_err(|e| format!("unable to serialize payload {}", e))?;
            Ok(BitcoinLumalanghookOccurrence::File(
                disk.path.to_string(),
                bytes,
            ))
        }
        HookAction::Noop => Ok(BitcoinLumalanghookOccurrence::Data(
            BitcoinLumalanghookOccurrencePayload::from_trigger(trigger),
        )),
    }
}
