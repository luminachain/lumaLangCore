...
use super::types::{
    append_error_context, BlockIdentifierIndexRule, LumalanghookInstance, ExactMatchingRule,
    HookAction,
};
...
pub struct StacksLumalanghookSpecification {
...
impl StacksLumalanghookSpecification {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if let Some(end_block) = self.end_block {
            ...
            if start_block > end_block {
                errors.push(
                    "LumaLang specification field `end_block` should be greater than `start_block`.".into()
                );
            }
            if (end_block - start_block) > MAX_BLOCK_HEIGHTS_ENTRIES {
                errors.push(format!("LumaLang specification exceeds max number of blocks to scan. Maximum: {}, Attempted: {}", MAX_BLOCK_HEIGHTS_ENTRIES, (end_block - start_block)));
            }
        }
        if errors.is_empty() {
            Ok(())
...
/// Maps some [StacksLumalanghookSpecification] to a corresponding [StacksNetwork]. This allows maintaining one
...
/// }
/// ```
/// You can deserialize the file to this type and create a [StacksLumalanghookInstance] for the desired network:
/// ```
/// use lumalanghook_sdk::lumalanghooks::stacks::StacksLumalanghookSpecificationNetworkMap;
/// use lumalanghook_sdk::lumalanghooks::stacks::StacksLumalanghookInstance;
/// use lumalanghook_types::StacksNetwork;
///
/// fn get_predicate(network: &StacksNetwork) -> Result<StacksLumalanghookInstance, String> {
///     let json_predicate =
///         std::fs::read_to_string("./predicate.json").expect("Unable to read file");
///     let hook_map: StacksLumalanghookSpecificationNetworkMap =
///         serde_json::from_str(&json_predicate).expect("Unable to parse LumaLang map");
///     hook_map.into_specification_for_network(network)
/// }
///
/// ```
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, JsonSchema)]
pub struct StacksLumalanghookSpecificationNetworkMap {
    pub uuid: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner_uuid: Option<String>,
    pub name: String,
    pub version: u32,
    pub networks: BTreeMap<StacksNetwork, StacksLumalanghookSpecification>,
}

impl StacksLumalanghookSpecificationNetworkMap {
    pub fn into_specification_for_network(
        mut self,
        network: &StacksNetwork,
    ) -> Result<StacksLumalanghookInstance, String> {
        let spec = self
            .networks
            .remove(network)
            .ok_or("Network unknown".to_string())?;
        Ok(StacksLumalanghookInstance {
            uuid: self.uuid,
...
pub struct StacksLumalanghookInstance {
    pub uuid: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub owner_uuid: Option<String>,
    pub name: String,
...
impl StacksLumalanghookInstance {
    pub fn key(&self) -> String {
        LumalanghookInstance::stacks_key(&self.uuid)
    }
...
#[derive(Clone)]
pub struct StacksTriggerLumalanghook<'a> {
    pub lumaLang: &'a StacksLumalanghookInstance,
...
#[derive(Clone, Debug)]
pub struct StacksLumalanghookPayload {
    pub uuid: String,
}

#[derive(Clone, Debug)]
pub struct StacksLumalanghookOccurrencePayload {
    ...
    pub lumaLang: StacksLumalanghookPayload,
}

impl StacksLumalanghookOccurrencePayload {
    pub fn from_trigger(trigger: StacksTriggerLumalanghook<'_>) -> StacksLumalanghookOccurrencePayload {
        StacksLumalanghookOccurrencePayload {
            apply: trigger
                .apply
                .into_iter()
            ...
            rollback: trigger
                ...
                .collect::<Vec<_>>(),
            lumaLang: StacksLumalanghookPayload {
                uuid: trigger.lumaLang.uuid.clone(),
            },
            events: trigger.events.into_iter().cloned().collect::<Vec<_>>(),
        }
    }
}
pub enum StacksLumalanghookOccurrence {
    Http(RequestBuilder, StacksLumalanghookOccurrencePayload),
    File(String, Vec<u8>),
    Data(StacksLumalanghookOccurrencePayload),
}

impl<'a> StacksTriggerLumalanghook<'a> {
    pub fn should_decode_clarity_value(&self) -> bool {
        self.lumaLang.decode_clarity_values.unwrap_or(false)
    }
}

pub fn evaluate_stacks_lumalanghooks_on_chain_event<'a>(
    chain_event: &'a StacksChainEvent,
    active_lumalanghooks: Vec<&'a StacksLumalanghookInstance>,
    ctx: &Context,
) -> (
    Vec<StacksTriggerLumalanghook<'a>>,
        ...
        StacksChainEvent::ChainUpdatedWithBlocks(update) => {
            for lumaLang in active_lumalanghooks.iter() {
                ...
                for block_update in update.new_blocks.iter() {
                        ...
                        block_update.parent_microblocks_to_apply.iter()
                    {
                        let (mut occurrences, mut expirations) =
                            evaluate_stacks_lumalanghook_on_blocks(
                    ...
                    let (mut occurrences, mut expirations) = evaluate_stacks_lumalanghook_on_blocks(
                ...
                if !apply.is_empty() || !rollback.is_empty() {
                    triggered_predicates.push(StacksTriggerLumalanghook {
                        lumaLang,
                        apply,
                        rollback,
                        events: vec![],
            ...
            if let Some(first_event) = data.events.first() {
                for lumaLang in active_lumalanghooks.iter() {
                    evaluated_predicates
                        .insert(lumaLang.uuid.as_str(), &first_event.received_at_block);
...
pub fn evaluate_stacks_lumalanghook_on_blocks<'a>(
    blocks: Vec<&'a dyn AbstractStacksBlock>,
    lumaLang: &'a StacksLumalanghookInstance,
    ctx: &Context,
) -> (
    ...
    let mut expired_predicates = BTreeMap::new();
    let end_block = lumaLang.end_block.unwrap_or(u64::MAX);
    for block in blocks {
        if end_block >= block.get_identifier().index {
            let mut hits = vec![];
            if lumaLang.is_predicate_targeting_block_header() {
                if evaluate_stacks_predicate_on_block(block, lumaLang, ctx) {
                ...
                for tx in block.get_transactions().iter() {
                    if evaluate_stacks_predicate_on_transaction(tx, lumaLang, ctx) {
                        hits.push(tx);
                    }
                }
            }
            ...
            expired_predicates.insert(lumaLang.uuid.as_str(), block.get_identifier());
        }
    }
...
fn serialize_stacks_non_consensus_event(
    ...
    use lumalanghook_types::StacksNonConsensusEventPayloadData;
...
pub fn serialize_stacks_payload_to_json<'a>(
    trigger: StacksTriggerLumalanghook<'a>,
    _proofs: &HashMap<&'a TransactionIdentifier, String>,
    ctx: &Context,
) -> JsonValue {
    let decode_clarity_values = trigger.should_decode_clarity_value();
    let include_contract_abi = trigger.lumaLang.include_contract_abi.unwrap_or(false);
    json!({
        ...
        "lumaLang": {
            "uuid": trigger.lumaLang.uuid,
            "predicate": trigger.lumaLang.predicate,
            "is_streaming_blocks": trigger.lumaLang.enabled
        }
    })
}
...
pub fn handle_stacks_hook_action<'a>(
    ...
    ctx: &Context,
) -> Result<StacksLumalanghookOccurrence, String> {
    match &trigger.lumaLang.action {
        HookAction::HttpPost(http) => {
            ...
            Ok(StacksLumalanghookOccurrence::Http(
                client
                    .request(method, &host)
                    .header("Content-Type", "application/json")
                    .header("Authorization", http.authorization_header.clone())
                    .body(body),
                StacksLumalanghookOccurrencePayload::from_trigger(trigger),
            ))
        }
        HookAction::FileAppend(disk) => {
            let bytes = serde_json::to_vec(&serialize_stacks_payload_to_json(trigger, proofs, ctx))
                .map_err(|e| format!("unable to serialize payload {}", e))?;
            Ok(StacksLumalanghookOccurrence::File(
                disk.path.to_string(),
                bytes,
            ))
        }
        HookAction::Noop => Ok(StacksLumalanghookOccurrence::Data(
            StacksLumalanghookOccurrencePayload::from_trigger(trigger),
        )),
    }
}
