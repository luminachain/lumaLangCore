...
use lumalanghook_types::{BitcoinNetwork, StacksNetwork};
...
pub struct LumalanghookStore {
    pub stacks_lumalanghooks: Vec<StacksLumalanghookInstance>,
    pub bitcoin_lumalanghooks: Vec<BitcoinLumalanghookInstance>,
}

impl Default for LumalanghookStore {
    fn default() -> Self {
        Self::new()
    }
}

impl LumalanghookStore {
    pub fn new() -> LumalanghookStore {
        LumalanghookStore {
            stacks_lumalanghooks: vec![],
            bitcoin_lumalanghooks: vec![],
        }
    }
    ...
    pub fn register_instance_from_network_map(
        ...
        hook: LumalanghookSpecificationNetworkMap,
    ) -> Result<LumalanghookInstance, String> {
        let spec = match hook {
            LumalanghookSpecificationNetworkMap::Stacks(hook) => {
                let spec = hook.into_specification_for_network(networks.1)?;
                self.stacks_lumalanghooks.push(spec.clone());
                LumalanghookInstance::Stacks(spec)
            }
            LumalanghookSpecificationNetworkMap::Bitcoin(hook) => {
                let spec = hook.into_specification_for_network(networks.0)?;
                self.bitcoin_lumalanghooks.push(spec.clone());
                LumalanghookInstance::Bitcoin(spec)
            }
        };
        Ok(spec)
    }

    pub fn enable_instance(&mut self, predicate_spec: &mut LumalanghookInstance) {
        match predicate_spec {
            LumalanghookInstance::Stacks(spec_to_enable) => {
                for spec in self.stacks_lumalanghooks.iter_mut() {
            ...
            LumalanghookInstance::Bitcoin(spec_to_enable) => {
                for spec in self.bitcoin_lumalanghooks.iter_mut() {
                    if spec.uuid.eq(&spec_to_enable.uuid) {
                        spec.enabled = true;
                        spec_to_enable.enabled = true;
                        break;
    ...
    pub fn register_instance(&mut self, spec: LumalanghookInstance) -> Result<(), String> {
        match spec {
            LumalanghookInstance::Stacks(spec) => {
                let spec = spec.clone();
                self.stacks_lumalanghooks.push(spec);
            }
            LumalanghookInstance::Bitcoin(spec) => {
                let spec = spec.clone();
                self.bitcoin_lumalanghooks.push(spec);
            }
        };
        Ok(())
    }

    pub fn deregister_stacks_hook(&mut self, hook_uuid: String) -> Option<StacksLumalanghookInstance> {
        let mut i = 0;
        while i < self.stacks_lumalanghooks.len() {
            if self.stacks_lumalanghooks[i].uuid == hook_uuid {
                let hook = self.stacks_lumalanghooks.remove(i);
                return Some(hook);
            } else {
                i += 1;
            }
    ...
    pub fn deregister_bitcoin_hook(
        &mut self,
        hook_uuid: String,
    ) -> Option<BitcoinLumalanghookInstance> {
        let mut i = 0;
        while i < self.bitcoin_lumalanghooks.len() {
            if self.bitcoin_lumalanghooks[i].uuid == hook_uuid {
                let hook = self.bitcoin_lumalanghooks.remove(i);
                return Some(hook);
            } else {
                i += 1;
            }
    ...
    pub fn expire_stacks_hook(&mut self, hook_uuid: String, block_height: u64) {
        let mut i = 0;
        while i < self.stacks_lumalanghooks.len() {
            if LumalanghookInstance::stacks_key(&self.stacks_lumalanghooks[i].uuid) == hook_uuid {
                self.stacks_lumalanghooks[i].expired_at = Some(block_height);
                break;
            } else {
                i += 1;
            }
    ...
    pub fn expire_bitcoin_hook(&mut self, hook_uuid: String, block_height: u64) {
        let mut i = 0;
        while i < self.bitcoin_lumalanghooks.len() {
            if LumalanghookInstance::bitcoin_key(&self.bitcoin_lumalanghooks[i].uuid) == hook_uuid {
                self.bitcoin_lumalanghooks[i].expired_at = Some(block_height);
                break;
            } else {
                i += 1;
            }
...
impl Serialize for LumalanghookStore {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut seq = serializer.serialize_seq(Some(
            self.bitcoin_lumalanghooks.len() + self.stacks_lumalanghooks.len(),
        ))?;
        for lumaLang in self.bitcoin_lumalanghooks.iter() {
            seq.serialize_element(lumaLang)?;
        }
        for lumaLang in self.stacks_lumalanghooks.iter() {
            seq.serialize_element(lumaLang)?;
        }
        seq.end()
    }
...
pub enum LumalanghookInstance {
    Bitcoin(BitcoinLumalanghookInstance),
    Stacks(StacksLumalanghookInstance),
}

impl LumalanghookInstance {
    ...
    pub fn deserialize_specification(spec: &str) -> Result<LumalanghookInstance, String> {
        let spec: LumalanghookInstance = serde_json::from_str(spec)
...
pub enum LumalanghookSpecificationNetworkMap {
    Bitcoin(BitcoinLumalanghookSpecificationNetworkMap),
    Stacks(StacksLumalanghookSpecificationNetworkMap),
}

impl LumalanghookSpecificationNetworkMap {
    ...
    pub fn deserialize_specification(
        spec: &str,
        _key: &str,
    ) -> Result<LumalanghookSpecificationNetworkMap, String> {
        let spec: LumalanghookSpecificationNetworkMap = serde_json::from_str(spec)
            .map_err(|e| format!("unable to deserialize predicate {}", e))?;
        Ok(spec)
    }
}
