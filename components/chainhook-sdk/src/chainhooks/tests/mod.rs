...
use super::{
    stacks::{
        evaluate_stacks_lumalanghooks_on_chain_event, handle_stacks_hook_action,
        StacksLumalanghookInstance, StacksLumalanghookOccurrence, StacksContractCallBasedPredicate,
        ...
        StacksStxEventBasedPredicate, StacksTrait, StacksTriggerLumalanghook,
    },
    types::{ExactMatchingRule, FileHook},
};
use crate::{
    lumalanghooks::stacks::serialize_stacks_payload_to_json,
...
#[test_case(
    vec![vec![StacksTransactionEventPayload::FTTransferEvent(lumalanghook_types::FTTransferEventData {
        sender: "".to_string(),
        asset_class_identifier: "different-id".to_string(),
        amount: "".to_string(),
        recipient: "".to_string(),
    }), StacksTransactionEventPayload::FTTransferEvent(lumalanghook_types::FTTransferEventData {
        sender: "".to_string(),
...
#[test_case(
    vec![vec![StacksTransactionEventPayload::NFTTransferEvent(lumalanghook_types::NFTTransferEventData {
        sender: "".to_string(),
        asset_class_identifier: "different-id".to_string(),
        hex_asset_identifier: "different-id".to_string(),
        recipient: "".to_string(),
    }), StacksTransactionEventPayload::NFTTransferEvent(lumalanghook_types::NFTTransferEventData {
        sender: "".to_string(),
        asset_class_identifier: "asset-id".to_string(),
...
fn test_stacks_predicates(
    ...
    // Prepare predicate
    let lumaLang = StacksLumalanghookInstance {
        uuid: "".to_string(),
        owner_uuid: None,
        name: "".to_string(),
    ...
    let predicates = vec![&lumaLang];
    let (triggered, _predicates_evaluated, _expired) =
        evaluate_stacks_lumalanghooks_on_chain_event(&event, predicates, &Context::empty());
...
fn verify_optional_addition_of_contract_abi() {
    ...
    let mut contract_deploy_lumalanghook = StacksLumalanghookInstance {
    ...
    let contract_call_lumalanghook = StacksLumalanghookInstance {
    ...
    let predicates = vec![&contract_deploy_lumalanghook, &contract_call_lumalanghook];
    ...
    for t in triggered.into_iter() {
        ...
        let uuid = result.get("lumaLang").unwrap().get("uuid").unwrap();
    ...
    contract_deploy_lumalanghook.include_contract_abi = Some(false);
    let predicates = vec![&contract_deploy_lumalanghook, &contract_call_lumalanghook];
    let (triggered, _blocks, _) =
        evaluate_stacks_lumalanghooks_on_chain_event(&event, predicates, &Context::empty());
    assert_eq!(triggered.len(), 2);
...
fn test_stacks_hook_action_noop() {
    ...
    let trigger = StacksTriggerLumalanghook {
        lumaLang: &lumaLang,
    ...
    if let StacksLumalanghookOccurrence::Data(data) = occurrence {
...
fn test_stacks_hook_action_file_append() {
    ...
    let occurrence =
        handle_stacks_hook_action(trigger, &proofs, &EventObserverConfig::default(), &ctx).unwrap();
    if let StacksLumalanghookOccurrence::File(path, bytes) = occurrence {
