...
fn new_events_can_be_converted_into_lumalanghook_event(original_event: StacksTransactionEventPayload) {
    let new_event = create_new_event_from_stacks_event(original_event.clone());
    let event = new_event.into_lumalanghook_event().unwrap();
...
#[test]
fn into_lumalanghook_event_rejects_invalid_missing_event() {
...
fn parses_block_response_signer_message() {
    use lumalanghook_types::{BlockResponseData, StacksSignerMessage};
