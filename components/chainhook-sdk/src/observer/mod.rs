...
use crate::lumalanghooks::bitcoin::{
    evaluate_bitcoin_lumalanghooks_on_chain_event, handle_bitcoin_hook_action,
    BitcoinLumalanghookInstance, BitcoinLumalanghookOccurrence, BitcoinLumalanghookOccurrencePayload,
    BitcoinTriggerLumalanghook,
};
use crate::lumalanghooks::stacks::{
    evaluate_stacks_lumalanghooks_on_chain_event, handle_stacks_hook_action, StacksLumalanghookInstance,
    StacksLumalanghookOccurrence, StacksLumalanghookOccurrencePayload,
};
use crate::lumalanghooks::types::{
    LumalanghookInstance, LumalanghookSpecificationNetworkMap, LumalanghookStore,
};
...
pub enum DataHandlerEvent {
    Process(BitcoinLumalanghookOccurrencePayload),
    Terminate,
}
...
pub struct EventObserverConfig {
    pub registered_lumalanghooks: LumalanghookStore,
...
/// ## Examples
/// ```
/// use lumalanghook_sdk::observer::EventObserverConfig;
/// use lumalanghook_sdk::observer::EventObserverConfigBuilder;
///
...
pub struct EventObserverConfigBuilder {
    ...
    pub lumalanghook_stacks_block_ingestion_port: Option<u16>,
...
impl EventObserverConfigBuilder {
    pub fn new() -> Self {
        EventObserverConfigBuilder {
            ...
            lumalanghook_stacks_block_ingestion_port: None,
    ...
    /// Sets the port at which LumaLang will observer Stacks blockchain events. The Stacks node's config should have an events_observer
    /// entry matching this port in order to send block events the LumaLang.
    pub fn lumalanghook_stacks_block_ingestion_port(&mut self, port: u16) -> &mut Self {
        self.lumalanghook_stacks_block_ingestion_port = Some(port);
        self
    }

    /// Sets whether LumaLang should display Stacks ingestion logs.
...
/// ```
/// use lumalanghook_sdk::observer::EventObserverConfig;
/// use lumalanghook_sdk::observer::BitcoinEventObserverConfigBuilder;
///
/// fn get_config() -> Result<EventObserverConfig, String> {
...
impl EventObserverConfig {
    ...
    /// Adds a [LumalanghookInstance] to config's the registered lumaLang store, returning the updated config.
    pub fn register_lumalanghook_instance(
        &mut self,
        spec: LumalanghookInstance,
    ) -> Result<&mut Self, String> {
        let mut lumalanghook_config = LumalanghookStore::new();
        lumalanghook_config.register_instance(spec)?;
        self.registered_lumalanghooks = lumalanghook_config;

        Ok(self)
    }

    /// Adds a [BitcoinLumalanghookInstance] to the config's registered lumaLang store, returning the updated config.
    pub fn register_bitcoin_lumalanghook_instance(
        &mut self,
        spec: BitcoinLumalanghookInstance,
    ) -> Result<&mut Self, String> {
        self.register_lumalanghook_instance(LumalanghookInstance::Bitcoin(spec))
    }
    /// Adds a [StacksLumalanghookInstance] to the config's registered lumaLang store, returning the updated config.
    pub fn register_stacks_lumalanghook_instance(
        &mut self,
        spec: StacksLumalanghookInstance,
    ) -> Result<&mut Self, String> {
        self.register_lumalanghook_instance(LumalanghookInstance::Stacks(spec))
    }
    ...
    ///
    /// *Note: This is used by external crates, so it should not be removed, even if not used internally by LumaLang.*
            ...
            bitcoin_block_signaling: overrides
                ...
                .unwrap_or_else(|| {
                    BitcoinBlockSignaling::Stacks(StacksNodeConfig::new(
                        ...
                        overrides
                            .and_then(|c| c.lumalanghook_stacks_block_ingestion_port)
...
pub enum ObserverCommand {
    ...
    RegisterPredicate(LumalanghookSpecificationNetworkMap),
    EnablePredicate(LumalanghookInstance),
...
pub enum ObserverEvent {
    ...
    NotifyBitcoinTransactionProxied,
    PredicateRegistered(LumalanghookInstance),
    PredicateDeregistered(PredicateDeregisteredEvent),
    PredicateEnabled(LumalanghookInstance),
    BitcoinPredicateTriggered(BitcoinLumalanghookOccurrencePayload),
    StacksPredicateTriggered(StacksLumalanghookOccurrencePayload),
...
/// ### Examples
/// ```
/// use lumalanghook_sdk::observer::EventObserverBuilder;
/// use lumalanghook_sdk::observer::EventObserverConfig;
/// use lumalanghook_sdk::observer::ObserverCommand;
/// use lumalanghook_sdk::utils::Context;
/// use std::error::Error;
/// use std::sync::mpsc::{Receiver, Sender};
///
...
pub fn start_event_observer(
    ...
    match config.bitcoin_block_signaling {
        BitcoinBlockSignaling::ZeroMQ(ref url) => {
            ...
            let _ = hiro_system_kit::thread_named("LumaLang event observer")
                .spawn(move || {
                        ...
                        Err(e) => {
                            if let Some(tx) = observer_events_tx {
                                context_cloned.try_log(|logger| {
                                    slog::crit!(
                                        logger,
                                        "LumaLang event observer thread failed with error: {e}",
                                    )
                                });
...
pub async fn start_bitcoin_event_observer(
    ...
    let lumalanghook_store = config.registered_lumalanghooks.clone();
    #[cfg(feature = "zeromq")]
    {
    ...
    prometheus_monitoring.initialize(
        lumalanghook_store.stacks_lumalanghooks.len() as u64,
        lumalanghook_store.bitcoin_lumalanghooks.len() as u64,
        None,
    );
...
pub fn gather_proofs<'a>(
    trigger: &BitcoinTriggerLumalanghook<'a>,
...
pub async fn start_observer_commands_handler(
    config: EventObserverConfig,
    mut lumalanghook_store: LumalanghookStore,
    observer_commands_rx: Receiver<ObserverCommand>,
    observer_events_tx: Option<crossbeam_channel::Sender<ObserverEvent>>,
    ...
    let mut lumalanghooks_occurrences_tracker: HashMap<String, u64> = HashMap::new();
    ...
    loop {
        ...
        match command {
            ...
            ObserverCommand::PropagateBitcoinChainEvent(blockchain_event) => {
                ...
                let bitcoin_lumalanghooks = lumalanghook_store
                    .bitcoin_lumalanghooks
                    .iter()
                    .filter(|p| p.enabled)
                ...
                ctx.try_log(|logger| {
                    slog::info!(
                        logger,
                        "Evaluating {} bitcoin lumalanghooks registered",
                        bitcoin_lumalanghooks.len()
                    )
                });

                let (predicates_triggered, predicates_evaluated, predicates_expired) =
                    evaluate_bitcoin_lumalanghooks_on_chain_event(
                        &chain_event,
                        &bitcoin_lumalanghooks,
                        &ctx,
                    );
                ...
                for entry in predicates_triggered.iter() {
                    ...
                    report.track_trigger(&entry.lumaLang.uuid, &blocks_ids);
                }

                ctx.try_log(|logger| {
                    slog::info!(
                        logger,
                        "{} bitcoin lumalanghooks positive evaluations",
                        predicates_triggered.len()
                    )
                });

                let mut lumalanghooks_to_trigger = vec![];

                for trigger in predicates_triggered.into_iter() {
                    let mut total_occurrences: u64 = *lumalanghooks_occurrences_tracker
                        .get(&trigger.lumaLang.uuid)
                        .unwrap_or(&0);
                    ...
                    let limit = trigger.lumaLang.expire_after_occurrence.unwrap_or(0);
                    if limit == 0 || total_occurrences <= limit {
                        lumalanghooks_occurrences_tracker
                            .insert(trigger.lumaLang.uuid.clone(), total_occurrences);
                        lumalanghooks_to_trigger.push(trigger);
                    } else {
                        hooks_ids_to_deregister.push(trigger.lumaLang.uuid.clone());
                    }
                }

                let mut proofs = HashMap::new();
                for trigger in lumalanghooks_to_trigger.iter() {
                    if trigger.lumaLang.include_proof {
                ...
                ctx.try_log(|logger| {
                    slog::info!(
                        logger,
                        "{} bitcoin lumalanghooks will be triggered",
                        lumalanghooks_to_trigger.len()
                    )
                });
                ...
                for lumalanghook_to_trigger in lumalanghooks_to_trigger.into_iter() {
                    let predicate_uuid = &lumalanghook_to_trigger.lumaLang.uuid;
                    match handle_bitcoin_hook_action(lumalanghook_to_trigger, &proofs, &config) {
                        Err(e) => {
                        ...
                        Ok(BitcoinLumalanghookOccurrence::Http(request, data)) => {
                            requests.push((request, data));
                        }
                        Ok(BitcoinLumalanghookOccurrence::File(_path, _bytes)) => {
                            ctx.try_log(|logger| {
                                slog::warn!(logger, "Writing to disk not supported in server mode")
                            })
                        }
                        Ok(BitcoinLumalanghookOccurrence::Data(payload)) => {
                            if let Some(ref tx) = observer_events_tx {
                ...
                for (request, data) in requests.into_iter() {
                    match send_request(request, 3, 1, &ctx).await {
                        ...
                        Err(e) => {
                            lumalanghook_store.deregister_bitcoin_hook(data.lumaLang.uuid.clone());
                            if let Some(ref tx) = observer_events_tx {
                                let _ = tx.send(ObserverEvent::PredicateInterrupted(PredicateInterruptedData {
                                    predicate_key: LumalanghookInstance::bitcoin_key(&data.lumaLang.uuid),
            ...
            ObserverCommand::PropagateStacksChainEvent(chain_event) => {
                ...
                let stacks_lumalanghooks = lumalanghook_store
                    .stacks_lumalanghooks
                    .iter()
                    .filter(|p| p.enabled)
                    .filter(|p| p.expired_at.is_none())
                    .collect::<Vec<_>>();
                ctx.try_log(|logger| {
                    slog::info!(
                        logger,
                        "Evaluating {} stacks lumalanghooks registered",
                        stacks_lumalanghooks.len()
                    )
                });
                ...
                // process hooks
                let (predicates_triggered, predicates_evaluated, predicates_expired) =
                    evaluate_stacks_lumalanghooks_on_chain_event(
                        &chain_event,
                        stacks_lumalanghooks,
                        &ctx,
                    );
                ...
                for entry in predicates_triggered.iter() {
                    ...
                    report.track_trigger(&entry.lumaLang.uuid, &block_ids);
                }
                ctx.try_log(|logger| {
                    slog::info!(
                        logger,
                        "{} stacks lumalanghooks positive evaluations",
                        predicates_triggered.len()
                    )
                });
                ...
                for lumalanghook_to_trigger in lumalanghooks_to_trigger.into_iter() {
                    let predicate_uuid = &lumalanghook_to_trigger.lumaLang.uuid;
                    match handle_stacks_hook_action(lumalanghook_to_trigger, &proofs, &config, &ctx) {
                        Err(e) => {
                            ctx.try_log(|logger| {
                                // todo: we may want to set predicates that reach this branch as interrupted,
                                // but for now we will error to see if this problem occurs.
                        ...
                        Ok(StacksLumalanghookOccurrence::Http(request, data)) => {
                            requests.push((request, data));
                        }
                        Ok(StacksLumalanghookOccurrence::File(_path, _bytes)) => {
                            ctx.try_log(|logger| {
                                slog::warn!(logger, "Writing to disk not supported in server mode")
                            })
                        }
                        Ok(StacksLumalanghookOccurrence::Data(payload)) => {
                            if let Some(ref tx) = observer_events_tx {
                ...
                for (request, data) in requests.into_iter() {
                    ...
                    ctx.try_log(|logger| {
                        slog::debug!(
                            logger,
                            "Dispatching request from stacks lumaLang {:?}",
                            request
                        )
                    });
                    match send_request(request, 3, 1, &ctx).await {
                        ...
                        Err(e) => {
                            lumalanghook_store.deregister_stacks_hook(data.lumaLang.uuid.clone());
                            if let Some(ref tx) = observer_events_tx {
                                let _ = tx.send(ObserverEvent::PredicateInterrupted(PredicateInterruptedData {
                                    predicate_key: LumalanghookInstance::stacks_key(&data.lumaLang.uuid),
                                    error: format!("Unable to evaluate predicate on Bitcoin chainstate: {}", e)
                                }));
                            }
                        }
            ...
            ObserverCommand::RegisterPredicate(spec) => {
                ...
                let mut spec =
                    match lumalanghook_store.register_instance_from_network_map(networks, spec) {
                        Ok(spec) => spec,
                        Err(e) => {
                            ctx.try_log(|logger| {
                                slog::warn!(
                                    logger,
                                    "Unable to register new lumaLang spec: {}",
                                    e.to_string()
                                )
                            });
                            continue;
                ...
                match spec {
                    LumalanghookInstance::Bitcoin(_) => {
                        prometheus_monitoring.btc_metrics_register_predicate()
                    }
                    LumalanghookInstance::Stacks(_) => {
                        prometheus_monitoring.stx_metrics_register_predicate()
                    }
                };

                ctx.try_log(
                    |logger| slog::debug!(logger, "Registering lumaLang {}", spec.uuid(),),
                );
                if let Some(ref tx) = observer_events_tx {
                    ...
                    lumalanghook_store.enable_instance(&mut spec);
                }
            }
            ...
            ObserverCommand::DeregisterStacksPredicate(hook_uuid) => {
                ...
                let hook = lumalanghook_store.deregister_stacks_hook(hook_uuid.clone());

                if hook.is_some() {
                    // on startup, only the predicates in the `lumalanghook_store` are added to the monitoring count,
                    // so only those that we find in the store should be removed
                    prometheus_monitoring.stx_metrics_deregister_predicate();
                };
                // event if the predicate wasn't in the `lumalanghook_store`, propogate this event to delete from redis
                if let Some(tx) = &observer_events_tx {
                    let _ = tx.send(ObserverEvent::PredicateDeregistered(
                        PredicateDeregisteredEvent {
            ...
            ObserverCommand::DeregisterBitcoinPredicate(hook_uuid) => {
                ...
                let hook = lumalanghook_store.deregister_bitcoin_hook(hook_uuid.clone());
                ...
                // even if the predicate wasn't in the `lumalanghook_store`, propogate this event to delete from redis
                if let Some(tx) = &observer_events_tx {
                    let _ = tx.send(ObserverEvent::PredicateDeregistered(
                        PredicateDeregisteredEvent {
                            predicate_uuid: hook_uuid.clone(),
            ...
            ObserverCommand::ExpireStacksPredicate(HookExpirationData {
                ...
                lumalanghook_store.expire_stacks_hook(hook_uuid, block_height);
            }
            ObserverCommand::ExpireBitcoinPredicate(HookExpirationData {
                ...
                lumalanghook_store.expire_bitcoin_hook(hook_uuid, block_height);
            }
        }
    }
