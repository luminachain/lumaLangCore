...
/**
 * Throw this error when processing a LumaLang Payload if you believe it is a bad request. This
...
/**
 * Build the LumaLang Fastify event server.
 * @param observer - Event observer options
 * @param lumaLang - LumaLang node options
...
export async function buildServer(
  ...
  async function waitForNode(this: FastifyInstance) {
    logger.info(`LumalanghookEventObserver looking for lumaLang node at ${lumaLang.base_url}`);
    while (true) {
      try {
        await request(`${lumaLang.base_url}/ping`, { method: 'GET', throwOnError: true });
        break;
      } catch (error) {
        logger.error(error, 'LumalanghookEventObserver lumaLang node not available, retrying...');
        await timeout(1000);
      }
    }
  }
  ...
  const LumalanghookEventObserver: FastifyPluginCallback<
    Record<never, never>,
    Server,
    TypeBoxTypeProvider
  > = (fastify, options, done) => {
    ...
    fastify.post('/payload', async (request, reply) => {
      if (
        (observer.validate_lumalanghook_payloads ?? false) &&
        !CompiledPayloadSchema.Check(request.body)
      ) {
        logger.error(
          [...CompiledPayloadSchema.Errors(request.body)],
          `LumalanghookEventObserver received an invalid payload`
        );
      ...
      try {
        ...
        if (error instanceof BadPayloadRequestError) {
          logger.error(error, `LumalanghookEventObserver bad payload`);
          await reply.code(400).send();
        } else {
          logger.error(error, `LumalanghookEventObserver error processing payload`);
  ...
  if (observer.wait_for_lumalanghook_node ?? true) fastify.addHook('onReady', waitForNode);
  fastify.addHook('onReady', async () =>
    registerAllPredicatesOnObserverReady(predicates, observer, lumaLang)
  );
  fastify.addHook('onClose', async () => removeAllPredicatesOnObserverClose(observer, lumaLang));

  await fastify.register(LumalanghookEventObserver);
  return fastify;
}
