...
use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinLumalanghookSpecification;
use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinLumalanghookSpecificationNetworkMap;
use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinPredicateType;
use lumalanghook_sdk::lumalanghooks::bitcoin::InscriptionFeedData;
use lumalanghook_sdk::lumalanghooks::bitcoin::OrdinalOperations;
use lumalanghook_sdk::lumalanghooks::stacks::StacksLumalanghookSpecification;
use lumalanghook_sdk::lumalanghooks::stacks::StacksLumalanghookSpecificationNetworkMap;
use lumalanghook_sdk::lumalanghooks::stacks::StacksPredicate;
use lumalanghook_sdk::lumalanghooks::stacks::StacksPrintEventBasedPredicate;
use lumalanghook_sdk::lumalanghooks::types::{LumalanghookSpecificationNetworkMap, FileHook, HookAction};
use lumalanghook_sdk::try_info;
use lumalanghook_sdk::types::{BitcoinNetwork, BlockIdentifier, StacksNetwork};
use lumalanghook_sdk::utils::{BlockHeights, Context};
...
struct ScanPredicate {
    /// LumaLang spec file to scan (json format)
...
struct CheckPredicate {
    /// LumaLang spec file to check (json format)
    pub predicate_path: String,
    /// Target Testnet network
    #[clap(long = "testnet", conflicts_with = "mainnet")]
    pub testnet: bool,
...
struct StartCommand {
    ...
    pub config_path: Option<String>,
    /// Specify relative path of the lumalanghooks (yaml format) to evaluate
    ...
    match opts.command {
        Command::Service(subcmd) => match subcmd {
            ServiceCommand::Start(cmd) => {
                ...
                let predicates = cmd
                    ...
                    .collect::<Result<Vec<LumalanghookSpecificationNetworkMap>, _>>()?;

                try_info!(ctx, "Starting lumaLang service");
        ...
        Command::Config(subcmd) => match subcmd {
            ConfigCommand::New(cmd) => {
                ...
                file_path.push("LumaLang.toml");
                let mut file = File::create(&file_path)
                    .map_err(|e| format!("unable to open file {}\n{}", file_path.display(), e))?;
                file.write_all(config_content.as_bytes())
                    .map_err(|e| format!("unable to write file {}\n{}", file_path.display(), e))?;
                println!("Created file LumaLang.toml");
            }
        },
        Command::Predicates(subcmd) => match subcmd {
            PredicatesCommand::New(cmd) => {
                ...
                let predicate = match (cmd.stacks, cmd.bitcoin) {
                    (true, false) => {
                        ...
                        networks.insert(StacksNetwork::Testnet, StacksLumalanghookSpecification {
                        ...
                        networks.insert(StacksNetwork::Mainnet, StacksLumalanghookSpecification {
                            start_block: Some(34239),
                            end_block: Some(50000),
                            blocks: None,
                            predicate: StacksPredicate::PrintEvent(StacksPrintEventBasedPredicate::Contains {
                        ...
                        LumalanghookSpecificationNetworkMap::Stacks(
                    ...
                    (false, true) => {
                        ...
                        LumalanghookSpecificationNetworkMap::Bitcoin(
            ...
            PredicatesCommand::Scan(cmd) => {
                ...
                predicate.validate()?;
                match predicate {
                    LumalanghookSpecificationNetworkMap::Bitcoin(predicate) => {
                    ...
                    LumalanghookSpecificationNetworkMap::Stacks(predicate) => {
                        let predicate_spec = match predicate
            ...
            PredicatesCommand::Check(cmd) => {
                ...
                let predicate: LumalanghookSpecificationNetworkMap =
...
pub fn load_predicate_from_path(
    ...
    file_reader
        ...
        .map_err(|e| format!("unable to read file {}\n{:?}", predicate_path, e))?;
    let predicate: LumalanghookSpecificationNetworkMap = serde_json::from_slice(&file_buffer)
