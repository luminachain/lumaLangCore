...
use lumalanghook_sdk::lumalanghooks::types::{LumalanghookSpecificationNetworkMap, LumalanghookStore};

use lumalanghook_sdk::lumalanghooks::types::LumalanghookInstance;
use lumalanghook_sdk::observer::{
...
use lumalanghook_sdk::{try_error, try_info};
use lumalanghook_sdk::types::{Chain, StacksBlockData, StacksChainEvent};
...
impl Service {
    ...
    pub async fn run(
        &mut self,
        predicates_from_startup: Vec<LumalanghookSpecificationNetworkMap>,
        observer_commands_tx_rx: Option<(Sender<ObserverCommand>, Receiver<ObserverCommand>)>,
    ) -> Result<(), String> {
        let mut lumalanghook_store = LumalanghookStore::new();

        // store all predicates from Redis that were in the process of scanning when
        // lumaLang was shutdown - we need to resume where we left off
        ...
        if self.config.is_http_api_enabled() {
            ...
            for (predicate, status) in registered_predicates.into_iter() {
                ...
                match lumalanghook_store.register_instance(predicate) {
                    Ok(_) => {
                        debug!(
                            self.ctx.expect_logger(),
        ...
        for predicate in predicates_from_startup.into_iter() {
            if let PredicatesApi::On(api_config) = &self.config.http_api {
                if let Ok(mut predicates_db_conn) = open_readwrite_predicates_db_conn(api_config) {
                    ...
                    if let Ok(Some(_)) = get_entry_from_predicates_db(
                        &LumalanghookInstance::either_stx_or_btc_key(uuid),
            ...
            match lumalanghook_store.register_instance_from_network_map(
                (
        ...
        event_observer_config.registered_lumalanghooks = lumalanghook_store;
        ...
        let predicate_api_shutdown = if let PredicatesApi::On(ref api_config) = config.http_api {
            info!(
                self.ctx.expect_logger(),
                "Listening on port {} for lumaLang predicate registrations", api_config.http_port
            );
            let ctx = self.ctx.clone();
        ...
        for predicate_with_last_scanned_block in leftover_scans {
            match predicate_with_last_scanned_block {
                (LumalanghookInstance::Stacks(spec), last_scanned_block) => {
                    let _ = stacks_scan_op_tx.send(StacksScanOp::StartScan {
                        predicate_spec: spec,
                        unfinished_scan_data: last_scanned_block,
                    });
                }
                (LumalanghookInstance::Bitcoin(spec), last_scanned_block) => {
        ...
        loop {
            ...
            match event {
                ObserverEvent::PredicateRegistered(spec) => {
                    ...
                    match spec {
                        LumalanghookInstance::Stacks(predicate_spec) => {
                            let _ = stacks_scan_op_tx.send(StacksScanOp::StartScan {
                                predicate_spec,
                                unfinished_scan_data: None,
                            });
                        }
                        LumalanghookInstance::Bitcoin(predicate_spec) => {
                            let _ = bitcoin_scan_op_tx.send(BitcoinScanOp::StartScan {
                                predicate_spec,
                                unfinished_scan_data: None,
                            });
                ...
                ObserverEvent::BitcoinChainEvent((chain_update, report)) => {
                    ...
                    if let PredicatesApi::On(ref config) = self.config.http_api {
                        ...
                        match chain_update {
                            lumalanghook_sdk::types::BitcoinChainEvent::ChainUpdatedWithBlocks(
                                data,
                            ) => {
                            ...
                            lumalanghook_sdk::types::BitcoinChainEvent::ChainUpdatedWithReorg(
                                data,
                            ) => {
                                for confirmed_block in &data.confirmed_blocks {
                                    if let Some(expired_predicate_uuids) =
