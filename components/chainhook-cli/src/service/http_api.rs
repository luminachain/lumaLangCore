...
fn handle_ping(ctx: &State<Context>) -> Json<JsonValue> {
    ...
    Json(json!({
        "status": 200,
        "result": "lumaLang service up and running",
    }))
}

#[openapi(tag = "Managing Predicates")]
#[get("/v1/lumalanghooks", format = "application/json")]
fn handle_get_predicates(
    api_config: &State<PredicatesApiConfig>,
    ctx: &State<Context>,
) -> Json<JsonValue> {
    ctx.try_log(|logger| slog::info!(logger, "Handling HTTP GET /v1/lumalanghooks"));
...
#[openapi(tag = "Managing Predicates")]
#[post("/v1/lumalanghooks", format = "application/json", data = "<predicate>")]
fn handle_create_predicate(
    predicate: Result<Json<LumalanghookSpecificationNetworkMap>, rocket::serde::json::Error>,
    api_config: &State<PredicatesApiConfig>,
    background_job_tx: &State<Arc<Mutex<Sender<ObserverCommand>>>>,
    ctx: &State<Context>,
) -> Json<JsonValue> {
    ctx.try_log(|logger| slog::info!(logger, "Handling HTTP POST /v1/lumalanghooks"));
    ...
    if let Ok(mut predicates_db_conn) = open_readwrite_predicates_db_conn(api_config) {
        if let Ok(Some(_)) = get_entry_from_predicates_db(
            &LumalanghookInstance::either_stx_or_btc_key(&predicate_uuid),
...
#[openapi(tag = "Managing Predicates")]
#[get("/v1/lumalanghooks/<predicate_uuid>", format = "application/json")]
fn handle_get_predicate(
    ...
    ctx.try_log(|logger| {
        slog::info!(
            logger,
            "Handling HTTP GET /v1/lumalanghooks/{}",
            predicate_uuid
        )
    });
...
#[openapi(tag = "Managing Predicates")]
#[delete("/v1/lumalanghooks/stacks/<predicate_uuid>", format = "application/json")]
fn handle_delete_stacks_predicate(
    ...
    ctx.try_log(|logger| {
        slog::info!(
            logger,
            "Handling HTTP DELETE /v1/lumalanghooks/stacks/{}",
            predicate_uuid
        )
    });
...
#[openapi(tag = "Managing Predicates")]
#[delete("/v1/lumalanghooks/bitcoin/<predicate_uuid>", format = "application/json")]
fn handle_delete_bitcoin_predicate(
    ...
    ctx.try_log(|logger| {
        slog::info!(
            logger,
            "Handling HTTP DELETE /v1/lumalanghooks/bitcoin/{}",
            predicate_uuid
        )
    });
...
pub fn get_entry_from_predicates_db(
    ...
    _ctx: &Context,
) -> Result<Option<(LumalanghookInstance, PredicateStatus)>, String> {
    let entry: HashMap<String, String> = predicate_db_conn.hgetall(predicate_key).map_err(|e| {
        format!(
            "unable to load lumaLang associated with key {}: {}",
            predicate_key,
            e
        )
    })?;
    ...
    let spec = LumalanghookInstance::deserialize_specification(encoded_spec)?;
...
pub fn get_entries_from_predicates_db(
    predicate_db_conn: &mut Connection,
    ctx: &Context,
) -> Result<Vec<(LumalanghookInstance, PredicateStatus)>, String> {
    let lumalanghooks_to_load: Vec<String> = predicate_db_conn
        .scan_match(LumalanghookInstance::either_stx_or_btc_key("*"))
        .map_err(|e| format!("unable to connect to redis: {}", e))?
        .collect();

    let mut predicates = vec![];
    for predicate_key in lumalanghooks_to_load.iter() {
        let lumaLang = match get_entry_from_predicates_db(predicate_key, predicate_db_conn, ctx) {
            Ok(Some((spec, status))) => (spec, status),
            Ok(None) => {
                warn!(
                    ctx.expect_logger(),
                    "unable to load lumaLang associated with key {}", predicate_key,
                );
                continue;
            }
            Err(e) => {
...
fn serialized_predicate_with_status(
    predicate: &LumalanghookInstance,
    status: &PredicateStatus,
) -> JsonValue {
    match (predicate, status) {
        (LumalanghookInstance::Stacks(spec), status) => json!({
            "chain": "stacks",
            "uuid": spec.uuid,
        ...
        (LumalanghookInstance::Bitcoin(spec), status) => json!({
            "chain": "bitcoin",
            "uuid": spec.uuid,
            "network": spec.network,
            "predicate": spec.predicate,
