use lumalanghook_sdk::lumalanghooks::types::LumalanghookSpecificationNetworkMap;
use lumalanghook_sdk::types::Chain;
...
use crate::service::tests::helpers::mock_service::{
    ...
    get_lumalanghook_config, get_predicate_status, setup_bitcoin_lumalanghook_test,
    setup_stacks_lumalanghook_test, TestSetupResult,
};
...
async fn await_new_scanning_status_complete(
    uuid: &str,
    lumalanghook_service_port: u16,
) -> Result<(), String> {
    let mut attempts = 0;
    loop {
        match get_predicate_status(uuid, lumalanghook_service_port).await? {
...
async fn test_stacks_predicate_status_is_updated(
    ...
    } = setup_stacks_lumalanghook_test(starting_chain_tip, None, None).await;
    ...
    let _ = call_register_predicate(&predicate, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))
        .unwrap();

    await_new_scanning_status_complete(uuid, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))
        .unwrap();
    ...
    let result = get_predicate_status(uuid, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))
        .unwrap();

    let found_predicate_status =
        filter_predicate_status_from_all_predicates(uuid, lumalanghook_service_port)
            .await
            .map_err(|e| cleanup_err(e, &working_dir, redis_port))
            .unwrap();
...
async fn test_bitcoin_predicate_status_is_updated(
    ...
    let TestSetupResult {
        ...
        prometheus_port: _,
        observer_command_tx: _,
    } = setup_bitcoin_lumalanghook_test(starting_chain_tip).await;
...
async fn test_bitcoin_predicate_status_is_updated_with_reorg(
    ...
    sleep(Duration::new(2, 0));
    let status = get_predicate_status(uuid, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))?;
...
async fn test_deregister_predicate(chain: Chain) -> Result<(), String> {
    let TestSetupResult {
        ...
        Chain::Stacks => setup_stacks_lumalanghook_test(3, None, None).await,
        Chain::Bitcoin => setup_bitcoin_lumalanghook_test(3).await,
    };
    ...
    let result = call_get_predicate(uuid, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))?;
    assert_eq!(result.get("status"), Some(&json!(200)));

    let result = call_deregister_predicate(&chain, uuid, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))?;
    assert_eq!(result.get("status"), Some(&json!(200)));
...
async fn test_restarting_with_saved_predicates(
    ...
    let TestSetupResult {
        ...
        prometheus_port: _,
        observer_command_tx: _,
    } = setup_stacks_lumalanghook_test(starting_chain_tip, Some((predicate, starting_status)), None)
        .await;
...
async fn it_allows_specifying_startup_predicate() -> Result<(), String> {
    ...
    let predicate =
        serde_json::from_value(predicate).expect("failed to set up stacks chanhook spec for test");
    let startup_predicate = LumalanghookSpecificationNetworkMap::Stacks(predicate);
    let TestSetupResult {
        working_dir,
        lumalanghook_service_port,
        redis_port,
        ...
        prometheus_port: _,
        observer_command_tx: _,
    } = setup_stacks_lumalanghook_test(3, None, Some(vec![startup_predicate])).await;
...
async fn register_predicate_responds_409_if_uuid_in_use() -> Result<(), String> {
    ...
    let startup_predicate = LumalanghookSpecificationNetworkMap::Stacks(stacks_spec);
    ...
    let result = call_register_predicate(&predicate, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))?;
...
fn it_generates_open_api_spec() {
    ...
    path.push("../../docs/lumaLang-openapi.json");
...
async fn it_seeds_block_pool_on_startup() -> Result<(), String> {
    ...
    let _ = start_lumalanghook_service(config, stacks_ingestion_port, None, &ctx).await;
