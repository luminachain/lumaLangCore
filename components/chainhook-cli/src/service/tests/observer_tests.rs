...
use lumalanghook_sdk::{
    lumalanghooks::types::LumalanghookStore,
...
use crate::service::tests::{
    ...
    setup_bitcoin_lumalanghook_test, setup_stacks_lumalanghook_test,
};
...
async fn ping_endpoint_returns_metrics() -> Result<(), String> {
    ...
    } = setup_stacks_lumalanghook_test(1, None, None).await;
...
async fn prometheus_endpoint_returns_encoded_metrics() -> Result<(), String> {
    ...
    let uuid = &get_random_uuid();
    let predicate = build_stacks_payload(Some("devnet"), None, None, None, Some(uuid));
    call_register_predicate(&predicate, lumalanghook_service_port)
        .await
        .map_err(|e| cleanup_err(e, &working_dir, redis_port))?;
    ...
    const EXPECTED: &str = "# HELP lumalanghook_stx_registered_predicates The number of Stacks predicates that have been registered by the LumaLang node.\n# TYPE lumalanghook_stx_registered_predicates gauge\nlumalanghook_stx_registered_predicates 1\n";
...
async fn bitcoin_rpc_requests_are_forwarded(endpoint: &str, body: Value) {
    let TestSetupResult {
        ...
        prometheus_port: _,
        observer_command_tx: _,
    } = setup_bitcoin_lumalanghook_test(1).await;
...
async fn it_responds_200_for_unimplemented_endpoints(
    ...
    let config = EventObserverConfig {
        ...
        bitcoin_block_signaling: lumalanghook_sdk::types::BitcoinBlockSignaling::Stacks(
            StacksNodeConfig {
                rpc_url: String::new(),
                ingestion_port,
            },
        ...
        stacks_network: lumalanghook_sdk::types::StacksNetwork::Devnet,
