...
use lumalanghook_sdk::lumalanghooks::types::PoxConfig;
use lumalanghook_sdk::observer::PredicatesConfig;
use lumalanghook_sdk::{
    lumalanghooks::stacks::StacksLumalanghookSpecificationNetworkMap,
    lumalanghooks::types::{LumalanghookInstance, LumalanghookSpecificationNetworkMap},
...
pub async fn call_register_predicate(
    ...
    let url = format!("http://localhost:{port}/v1/lumalanghooks");
...
pub async fn call_deregister_predicate(
    ...
    let url = format!("http://localhost:{port}/v1/lumalanghooks/{chain}/{predicate_uuid}");
...
pub async fn call_get_predicate(predicate_uuid: &str, port: u16) -> Result<JsonValue, String> {
    let url = format!("http://localhost:{port}/v1/lumalanghooks/{predicate_uuid}");
...
pub fn get_lumalanghook_config(
    redis_port: u16,
    lumalanghook_port: u16,
    stacks_rpc_port: u16,
...
pub async fn start_lumalanghook_service(
    config: Config,
    ping_startup_port: u16,
    startup_predicates: Option<Vec<LumalanghookSpecificationNetworkMap>>,
    ctx: &Context,
    ...
    let _ = hiro_system_kit::thread_named("LumaLang service")
        .spawn(move || {
        ...
        .map_err(|e| format!("failed to start lumaLang service thread, {}", e))?;
    ...
    loop {
        if attempts >= MAX_ATTEMPTS {
            return Err("failed to ping lumaLang service".to_string());
        }
...
pub struct TestSetupResult {
    ...
    pub lumalanghook_service_port: u16,
    pub redis_port: u16,
...
pub async fn setup_stacks_lumalanghook_test(
    starting_chain_tip: u64,
    redis_seed: Option<(StacksLumalanghookSpecificationNetworkMap, PredicateStatus)>,
    startup_predicates: Option<Vec<LumalanghookSpecificationNetworkMap>>,
) -> TestSetupResult {
    let (
        redis_port,
        ...
        bitcoin_rpc_port,
        prometheus_port,
    ) = setup_lumalanghook_service_ports().unwrap_or_else(|e| panic!("test failed with error: {e}"));
    ...
    if let Some((predicate, status)) = redis_seed {
        ...
        let spec = LumalanghookInstance::Stacks(stacks_spec);
    ...
    let observer_command_tx =
        start_lumalanghook_service(config, lumalanghook_service_port, startup_predicates, &ctx)
            .await
            .unwrap_or_else(|e| {
                std::fs::remove_dir_all(&working_dir).unwrap();
                flush_redis(redis_port);
...
pub async fn setup_bitcoin_lumalanghook_test(starting_chain_tip: u64) -> TestSetupResult {
    let (
        redis_port,
        lumalanghook_service_port,
        stacks_rpc_port,
    ...
    let config = get_lumalanghook_config(
        redis_port,
        lumalanghook_service_port,
        stacks_rpc_port,
        stacks_ingestion_port,
    ...
    let terminator_tx = start_lumalanghook_service(config, lumalanghook_service_port, None, &ctx)
        .await
        .unwrap_or_else(|e| {
            std::fs::remove_dir_all(&working_dir).unwrap();
            flush_redis(redis_port);
...
pub fn setup_lumalanghook_service_ports() -> Result<(u16, u16, u16, u16, u16, u16), String> {
    let redis_port = 6379;
    let lumalanghook_service_port = get_free_port()?;
