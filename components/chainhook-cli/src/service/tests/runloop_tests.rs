...
use lumalanghook_sdk::{
    lumalanghooks::{
        bitcoin::{BitcoinLumalanghookInstance, BitcoinPredicateType},
        stacks::{StacksLumalanghookInstance, StacksPredicate},
...
use crate::{
    ...
    service::{
        ...
        tests::helpers::{
            mock_bitcoin_rpc::mock_bitcoin_rpc, mock_service::setup_lumalanghook_service_ports,
        },
    },
};
...
async fn test_stacks_runloop_kill_scan() {
    ...
    let predicate_spec = StacksLumalanghookInstance {
        uuid: uuid.clone(),
        owner_uuid: None,
        name: "idc".to_string(),
    ...
    // this [issue](https://github.com/luminachain/lumaLang/issues/509) will give us
...
async fn test_stacks_bitcoin_kill_scan() {
    ...
    let uuid = "test".to_string();
    let predicate_spec = BitcoinLumalanghookInstance {
        uuid: uuid.clone(),
        owner_uuid: None,
        name: "idc".to_string(),
