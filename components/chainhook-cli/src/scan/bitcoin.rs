...
use lumalanghook_sdk::bitcoincore_rpc::RpcApi;
use lumalanghook_sdk::bitcoincore_rpc::{Auth, Client};
use lumalanghook_sdk::lumalanghooks::bitcoin::{
    evaluate_bitcoin_lumalanghooks_on_chain_event, handle_bitcoin_hook_action,
    BitcoinLumalanghookOccurrence, BitcoinTriggerLumalanghook,
};
use lumalanghook_sdk::lumalanghooks::bitcoin::BitcoinLumalanghookInstance;
use lumalanghook_sdk::indexer;
use lumalanghook_sdk::indexer::bitcoin::{
    build_http_client, download_and_parse_block_with_retry, retrieve_block_hash_with_retry,
};
use lumalanghook_sdk::indexer::fork_scratch_pad::CONFIRMED_SEGMENT_MINIMUM_LENGTH;
use lumalanghook_sdk::observer::{gather_proofs, EventObserverConfig};
use lumalanghook_sdk::types::{
    BitcoinBlockData, BitcoinChainEvent, BitcoinChainUpdatedWithBlocksData, BlockIdentifier, Chain,
};
use lumalanghook_sdk::utils::{file_append, send_request, Context};
...
pub async fn process_block_with_predicates(
    ...
    predicates: &Vec<&BitcoinLumalanghookInstance>,
...
pub async fn execute_predicates_action<'a>(
    ...
    for trigger in hits.into_iter() {
        ...
        let predicate_uuid = &trigger.lumaLang.uuid;
        match handle_bitcoin_hook_action(trigger, &proofs, &config) {
            ...
            Ok(action) => {
                actions_triggered += 1;
                match action {
                    BitcoinLumalanghookOccurrence::Http(request, _) => {
                        send_request(request, 10, 3, ctx).await?
                    }
                    BitcoinLumalanghookOccurrence::File(path, bytes) => {
                        file_append(path, bytes, ctx)?
                    }
                    BitcoinLumalanghookOccurrence::Data(_payload) => {}
                };
            }
        }
    }
