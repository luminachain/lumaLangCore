...
use lumalanghook_sdk::utils::{BlockHeights, BlockHeightsError};
        ...
        match BlockHeights::Blocks(blocks.clone()).get_sorted_entries() {
            ...
            Err(e) => match e {
                BlockHeightsError::ExceedsMaxEntries(max, specified) => {
                    return Err(format!("LumaLang specification exceeds max number of blocks to scan. Maximum: {}, Attempted: {}", max, specified));
                }
