...
use lumalanghook_sdk::types::{BitcoinNetwork, StacksNetwork};
...
fn config_from_file_matches_generator_for_all_networks(network: BitcoinNetwork) {
    ...
    let path = format!(
        "{}/src/config/tests/fixtures/{}_lumalanghook.toml",
        LOCAL_DIR,
        mode.as_str()
    );
...
fn config_from_file_allows_local_tsv_file() {
    let path = format!(
        "{}/src/config/tests/fixtures/local_tsv_lumalanghook.toml",
        LOCAL_DIR,
    );
...
fn parse_config_from_file_rejects_config_with_unsupported_mode() {
    let path = format!(
        "{}/src/config/tests/fixtures/unsupported_lumalanghook.toml",
        LOCAL_DIR
    );
...
fn it_has_default_config_for_each_network() {
    ...
    let path = format!(
        "{}/src/config/tests/fixtures/devnet_lumalanghook.toml",
        LOCAL_DIR
    );
