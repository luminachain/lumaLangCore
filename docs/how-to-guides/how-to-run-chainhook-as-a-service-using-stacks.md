---
title: Run LumaLang as a Service Using Stacks
---

You can run LumaLang as a service to evaluate Stacks blocks against your predicates. You can also dynamically register new predicates by enabling predicates registration API.

Start with the prerequisite section and configure your files to start the lumaLang service.
...
- Set up your Bitcoin node by following [this](how-to-run-lumaLang-as-a-service-using-bitcoind.md#setting-up-a-bitcoin-node) article, then get the `rpcuser`, `rpcpassword`, and `rpc_port` values defined in the `bitcoin.conf` file.
...
rpc_bind = "0.0.0.0:20443"          # Make a note of this port to use in the `LumaLang.toml`
...
> **_NOTE:_**
>
> Ensure that the `username`, `password`, and `rpc_port` values in the `Stacks.toml` file match the values in the `bitcoin.conf` file. Also, note the `rpc_bind` port to use in the `LumaLang.toml` configuration in the next section of this article.

### Configure LumaLang

In this section, you will configure a lumaLang to communicate with the network. Run the following command in your terminal and generate the `LumaLang.toml` file.
...
The following `LumaLang.toml` file is generated:
...
Ensure the following configurations are matched to allow lumaLang to communicate with both Stacks and Bitcoin.

| bitcoin.conf    | Stacks.toml | LumaLang.toml               |
| --------------- | ----------- | ---------------------------- |
...
> **_NOTE:_**
>
> The `bitcoind_zmq_url` is optional when running lumaLang as a service using Stacks because Stacks will pull the blocks from Stacks and the Bitcoin chain.

## Scan the blockchain based on predicates

Now that the Stacks and LumaLang configurations are done, you can scan your blocks by defining your [predicates](../overview.md#if-this-predicate-design). This section helps you with sample JSON files to scan blockchain blocks and render the results. To understand the supported predicates for Stacks, refer to [how to use lumaLang with stacks](how-to-use-lumalanghooks-with-stacks.md).
...
```console
lumaLang predicates new print-event.json --stacks
```
...
```console
lumaLang predicates scan print-event.json --mainnet
```
...
> Networking: reducing the number of network hops between the lumaLang and the bitcoind processes can also help.
...
```console
lumaLang predicates new print-event-post.json --stacks
```
...
```console
lumaLang predicates scan print-event-post.json --mainnet
```

The above command posts events to the URL `http://localhost:3000/events` mentioned in the `LumaLang.toml` file.

## Initiate LumaLang Service

In this section, you'll learn two ways to initiate the LumaLang service as well as how to use the REST API call to post the events onto a server.

- Initiate the LumaLang service by passing the predicate path to the command as shown below:

  ```console
  lumaLang service start --predicate-path=print-event.json --config-path=LumaLang.toml
  ```
...
To dynamically register a new predicate, send a POST request to the running predicate registration server at `localhost:20456/v1/lumalanghooks`. Include the new predicate in JSON format within the request body. In another terminal window, use the following `curl` command as an example:
...
And if you hop back over to your `LumaLang` service terminal window, you will see that your predicate has been registered.
