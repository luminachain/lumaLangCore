---
title: Use Lumalanghooks with Stacks
---

This guide helps you define predicates to use LumaLang with Stacks. The predicates are specified based on `if-this`, and `then-that` constructs.
...
Following additional configurations can be used to improve the performance of lumaLang by preventing a full scan of the blockchain:
...
- Stop evaluating lumaLang after a given number of occurrences found:
...
A specification file can also include different networks. In this case, the lumaLang will select the predicate corresponding to the network it was launched against.
