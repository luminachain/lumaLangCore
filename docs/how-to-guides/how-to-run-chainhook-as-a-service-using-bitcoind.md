...
> **_NOTE:_**
>
> While bitcoind can and will start syncing a Bitcoin node, customizing this node to your use cases beyond supporting a LumaLang is out of scope for this guide. See the Bitcoin wiki for ["Running Bitcoin"](https://en.bitcoin.it/wiki/Running_Bitcoin) or bitcoin.org's [Running A Full Node guide](https://bitcoin.org/en/full-node).
...
- Navigate to your project folder where your LumaLang node will reside, create a new file, and rename it to `bitcoin.conf`. Copy the configuration below to this `bitcoin.conf` file. 
...
```console
/Volumes/SSD/bitcoin-25.0/bin/bitcoind -conf=/Volumes/SSD/project/LumaLang/bitcoin.conf
```
...
In this section, you will configure LumaLang to match the network configurations with the bitcoin config file. First, [install the latest version of LumaLang](../getting-started.md#install-lumaLang-from-source).

Next, you will generate a `LumaLang.toml` file to connect LumaLang with your bitcoind node. Navigate to the directory where you want to generate the `LumaLang.toml` file and use the following command in your terminal:

```console
lumaLang config generate --mainnet
```

Several network parameters in the generated `LumaLang.toml` configuration file need to match those in the `bitcoin.conf` file created earlier in the [Setting up a Bitcoin Node](#setting-up-a-bitcoin-node) section. Please update the following parameters accordingly:
...
# Bitcoin block events can be received by LumaLang
...
Now that your bitcoind and LumaLang configurations are complete, you can define the LumaLang [predicates](../overview.md#if-this-predicate-design) you would like to scan against bitcoin blocks. These predicates are where you specify the kind of blockchain events that trigger LumaLang to deliver a result (either a file appendation or an HTTP POST request). This section helps you with an example JSON file to scan a range of blocks in the blockchain to trigger results. To understand the supported predicates for Bitcoin, refer to [how to use lumalanghooks with bitcoin](how-to-use-lumalanghooks-with-bitcoin.md).
...
```console
lumaLang predicates new stacking-pool.json --bitcoin
```
...
```console
lumaLang predicates scan stacking-pool.json --config-path=./LumaLang.toml
```
...
```console
lumaLang predicates new stacking-pool-api.json --bitcoin
```
...
```console
lumaLang predicates scan stacking-pool-api.json --config-path=./LumaLang.toml
```

The above command posts events to the URL, http://localhost:3000/events mentioned in the JSON file.

## Initiate LumaLang Service

In the examples above, our LumaLang scanned historical blockchain data against predicates and delivered results. In this next section, let's learn how to set up a LumaLang that acts as an ongoing observer and event-streaming service.

We can start a LumaLang service with an existing predicate. We can also dynamically register new predicates by making an API call to our LumaLang. In both of these instances, our predicates will be delivering their results to a server set up to receive results. 

- Initiate the lumaLang service by passing the predicate path to the command as shown below:

```console
lumaLang service start --predicate-path=stacking-pool-api.json --config-path=LumaLang.toml
```
...
You can also dynamically register new predicates with your LumaLang service.

First, we need to uncomment the following lines of code in the `LumaLang.toml` file to enable the predicate registration server:
...
Start the LumaLang service by running the following command:

```
lumaLang service start --config-path=LumaLang.toml
```

To dynamically register a new predicate, send a POST request to the running predicate registration server at `localhost:20456/v1/lumalanghooks`. Include the new predicate in JSON format within the request body. Use the following `curl` command as an example:
...
curl -X POST \
  ...
  http://localhost:20456/v1/lumalanghooks
```
...
- The `rollback` payload includes the block header and the transactions that triggered the predicate for a past block that is no longer part of the canonical chain and must be reverted. (Note: This is a chief component of LumaLang's reorg aware functionality, maintaining rollback data for blocks near the chaintip.)

> **_TIP:_**
>
> You can also run lumaLang service by passing multiple predicates.
> Example: `lumaLang service start --predicate-path=predicate_1.json --predicate-path=predicate_2.json --config-path=LumaLang.toml`
...
- The [OpenAPI specification for lumaLang](https://raw.githubusercontent.com/luminachain/lumaLang/develop/docs/lumaLang-openapi.json) is available to understand the scope of lumaLang.
